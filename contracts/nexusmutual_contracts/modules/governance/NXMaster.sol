...
contract NXMaster is Governed {
  ...
  function initiateMaster(address _tokenAdd) external {
    ...
    OwnedUpgradeabilityProxyV5 proxy = OwnedUpgradeabilityProxyV5(address(uint160(address(this))));
  ...
  onlyAuthorizedToGovern
  {
    ...
    for (uint i = 0; i < _contractNames.length; i++) {
      ...
      OwnedUpgradeabilityProxyV5 proxy = OwnedUpgradeabilityProxyV5(allContractVersions[_contractNames[i]]);
  ...
  function _generateProxy(address _implementationAddress) internal returns (address) {
    OwnedUpgradeabilityProxyV5 proxy = new OwnedUpgradeabilityProxyV5(_implementationAddress);
